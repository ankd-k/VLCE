...
import ExtPerspectiveCamera from './ext-perspective-camera';
...
class TextScene implements IRenderScene {
  ...
  constructor(renderer: THREE.WebGLRenderer, clock?: THREE.Clock, initText?: string) {
    ...
    this._camera = new ExtPerspectiveCamera(45, size.x / size.y, 0.1, 1000.0);
    ...
    this._cursor = new Cursor(this._scene);
  ...
  public change = (e: AceAjax.EditorChangeEvent, text: string) => {
    ...
    this._camera.cameraPos = this._cursor.meshPos.clone().add(new THREE.Vector3(0, 0, 50));
